...
class UserClientSerializerView(
    generics.ListAPIView,
    generics.CreateAPIView,
    generics.UpdateAPIView,
    generics.DestroyAPIView
):
    """CRUD Api for UserClientSerializerView model
    ...
    """
    queryset = (
        UserClient.custom_manager.alive()
    )
    ...
    def list(self, request, *args, **kwargs):
        response = super(UserClientSerializerView, self).list(request, *args, **kwargs)
        check = dict(response.data)
        if check['count'] == 0:
            return rest_default_response(data=None, status=status.HTTP_404_NOT_FOUND)
        return rest_default_response(data=response.data, status=status.HTTP_200_OK)
    ...
    def filter_queryset(self, queryset):
        ...
        if search_name:
            queryset = queryset.filter(
                Q(user__full_name__icontains=search_name) |
                Q(client__sap_card_name__icontains=search_name) |
                Q(client__sap_sap_card_code__icontains=search_name)
            )
