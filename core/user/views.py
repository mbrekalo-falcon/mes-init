from core.__init__ import *

from models.user.models import UserModel

from .serializers import UserSerializer, UserListSerializer, UserCreateSerializer

from core.helpers.enum import RegisterEnum
from core.helpers.mixin import generate_custom_uuid


class UserView(
    generics.ListAPIView,
    generics.CreateAPIView,
    generics.UpdateAPIView,
    generics.DestroyAPIView,
):
    queryset = UserModel.custom_manager.alive().filter(is_superuser=False)
    serializer_class = UserListSerializer
    pagination_class = StandardApplicationSetPagination
    filter_backends = (DjangoFilterBackend, )
    filter_fields = ('full_name', )
    permission_classes = (IsAuthenticated, )
    lookup_url_kwarg = ('instance_id', )

    def list(self, request, *args, **kwargs):
        response = super(UserView, self).list(request, *args, **kwargs)
        check = dict(response.data)
        if check['count'] == 0:
            return rest_default_response(data=None, status=status.HTTP_404_NOT_FOUND)
        return rest_default_response(data=response.data, status=status.HTTP_200_OK)

    def filter_queryset(self, queryset):
        row_size = self.request.GET.get('row_size')
        search_name = self.request.GET.get('search_name')
        instance_id = self.request.GET.get('instance_id')
        queryset = self.queryset

        if row_size:
            self.pagination_class.page_size = row_size

        if instance_id:
            queryset = queryset.filter(id=instance_id)

        if search_name:
            queryset = queryset.filter(full_name__icontains=search_name)

        return queryset

    def create(self, request, *args, **kwargs):
        data = request.data
        check_email = UserModel.custom_manager.alive().filter(email=data['email'].lower())
        if check_email.exists():
            return rest_default_response(message=RegisterEnum.EMAIL_EXISTS.value, status=status.HTTP_403_FORBIDDEN)

        data['username'] = str(generate_custom_uuid())
        data['is_active'] = True
        serializer = UserCreateSerializer(data=data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return rest_default_response(message=RegisterEnum.USER_SUCCESS.value, status=status.HTTP_201_CREATED)
        return rest_default_response(message=serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def update(self, request, *args, **kwargs):
        instance_id = self.request.GET.get('instance_id')
        if not instance_id:
            return rest_default_response(status=status.HTTP_403_FORBIDDEN, message='Please provide instance_id.')
        check_query = self.get_queryset().filter(id=instance_id)
        if not check_query.exists():
            return rest_default_response(status=status.HTTP_404_NOT_FOUND, message='Object not found.')

        serializer = UserSerializer(
            data=self.request.data,
            instance=check_query.first(),
            partial=True
        )
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            user_detail = serializer.data
            del user_detail['password']
            return rest_default_response(data=user_detail, status=status.HTTP_200_OK)
        return rest_default_response(message=serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def destroy(self, request, *args, **kwargs):
        instance_id = self.request.GET.get('instance_id')
        if not instance_id:
            return rest_default_response(status=status.HTTP_403_FORBIDDEN, message='Please provide instance_id.')
        check_query = self.get_queryset().filter(id=instance_id)
        if not check_query.exists():
            return rest_default_response(status=status.HTTP_404_NOT_FOUND, message='Object not found.')

        check_query.first().soft_delete()
        return rest_default_response(status=status.HTTP_200_OK, message='Object deleted.')
